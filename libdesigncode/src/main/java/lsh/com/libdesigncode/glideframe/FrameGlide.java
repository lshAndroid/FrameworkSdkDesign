 ...
 * Created by lsh on 2018/1/15.
 */

public class FrameGlide implements ImageEngine {
    //单例
    private static FrameGlide instance = null;
    //单例模式，私有构造方法
    private FrameGlide() {
    }
    //获取单例
    public static FrameGlide getInstance() {
        if (null == instance) {
            synchronized (FrameGlide.class) {
                if (null == instance) {
                    instance = new FrameGlide();
                }
            }
        }
        return instance;
